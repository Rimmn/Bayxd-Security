...
async function startBot() {
    ...
    const promptText =
        chalk.bgRed.bold(`\nRead Before Continuing ⚠️\n`) +
        chalk.white.bold(`• Pastikan nomor yang akan dipake sudah terdaftar di database script\n`) +
        chalk.white.bold(`• Jika belum ada di database script akan menolak!\n`) +
        chalk.white.bold(`• Hubungi owner jika belum add database\n\n`) +
...
nou.mem.info().then(memInfo => {
    ...
    nou.drive.info().then(tot => {
        ...
        console.log(line);
        console.log(chalk.magenta.bold('💬 Informasi Script'));
        console.log(line);
        console.log(chalk.white(`› Name Script  : ${global.namaBot}`));
        console.log(chalk.white(`› Developer    : ${global.namaOwner}`));
        console.log(chalk.white(`› Version      : ${global.versiBot}`));
        console.log(chalk.white(`› Type Script  : Case X Plugins`));

        console.log(line);
        console.log(chalk.blue.bold('💻 Informasi Server'));
        console.log(line);
        console.log(chalk.white(`› OS Platform  : ${nou.os.type()}`));
        console.log(chalk.white(`› RAM VPS      : ${usedGB}/${totalGB} GB used (${freeGB} GB free)`));
        console.log(chalk.white(`› Disk Space   : ${tot.usedGb}/${tot.totalGb} GB used`));
        console.log(chalk.white(`› CPU Core     : ${cpuCores} core(s)`));
        console.log(chalk.white(`› VPS Uptime   : ${vpsUptime}`));
        console.log(chalk.white(`› CPU Type     : ${os.cpus()[0].model}`));

        console.log(line);
        console.log(chalk.green.bold('[✓] - Script telah terkoneksi di WhatsApp anda. Selamat menggunakan!\n'));
    });
});
 }
});
