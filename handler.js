...
if (body.startsWith(prefix + command)) {
    if (global.pconly && m.isGroup) {
        return m.reply("Bot Hanya Dapat Digunakan di privat chat ⚠️");
    }
...
async function JadwalSholat(kota) {
  try {
    ...
    if (!dataKota) return `⚠️ Kota *${kota}* tidak ditemukan.`
...
if (m.isGroup && !m.key.fromMe) {
    ...
    for (const ment of mentionUser) {
        ...
        if (targetUser && targetUser.afk_time > -1) {
            ...
            await reply(`Eh, jangan di-tag orangnya lagi *AFK!* 😴
      
🕒 *Sejak:* ${anu2.hours} jam, ${anu2.minutes} menit, ${anu2.seconds} detik lalu
💬 *Alasan:* ${reason}`);
        }
    }
    ...
    if (senderUser && senderUser.afk_time > -1) {
        ...
        await sock.sendMessage(m.chat, {
            text: `Yeay! @${m.sender.split('@')[0]} udah balik dari *AFK!* 🎉

🕒 *Durasi:* ${anu.hours} jam, ${anu.minutes} menit, ${anu.seconds} detik
💬 *Alasan:* ${reason}`,
            mentions: [m.sender]
        }, { quoted: m });
    }
...
if (m.isGroup) {
  if (chatDb.antisticker === 1 && isBotAdmin) {
    if (m.message?.stickerMessage && !isOwner && !isSewa && !isAdmin && !m.key.fromMe) {
      ...
      await sock.sendMessage(m.chat, {
        text: `*Group Security!*
› Hii @${m.sender.split('@')[0]}, pesanmu dihapus.
› Jangan kirim *sticker* di sini!`,
        mentions: [m.sender]
      }, { quoted: m });
  ...
  if (chatDb.antilinkgc === 1) {
    ...
    if (regex.test(m.text) && !isOwner && !isSewa && !isAdmin && isBotAdmin && !m.key.fromMe) {
      ...
      await sock.sendMessage(m.chat, {
        text: `*Group Security!*
› Hii @${m.sender.split('@')[0]}, pesanmu dihapus 
› Jangan kirim link *grup lain* di sini!`,
        mentions: [m.sender]
      }, { quoted: m });
  ...
  if (chatDb.antilinkch === 1) {
    ...
    if (regex.test(m.text) && !isOwner && !isSewa && !isAdmin && isBotAdmin && !m.key.fromMe) {
      ...
      await sock.sendMessage(m.chat, {
        text: `*Group Security!*
› Hii @${m.sender.split('@')[0]}, pesanmu dihapus 
› Jangan kirim link *saluran WhatsApp* di sini!`,
        mentions: [m.sender]
      }, { quoted: m });
  ...
  if (chatDb.antitoxic === 1 && isBotAdmin) {
    ...
    if (Warn && !isOwner && !isSewa && !isAdmin && !m.key.fromMe) {
      ...
      await sock.sendMessage(m.chat, {
        text: `*Group Security!*
› Hii @${m.sender.split('@')[0]}, pesanmu dihapus  
› Kata terlarang tidak diperbolehkan di grup ini.`,
        mentions: [m.sender]
      }, { quoted: m });
      ...
      if (adaLink) {
        ...
        await sock.sendMessage(
          m.chat,
          {
            text: `*Group Security!*
› Hii @${m.sender.split('@')[0]}, pesanmu dihapus 
› Jangan kirim *link* apapun di sini!`,
            mentions: [m.sender],
          },
          { quoted: m }
        );
...
console.log(
    chalk.white.bold('\n'), chalk.bgGreen.bold('[✓] - MESSAGE DETECTED'),
    '\n' + chalk.white('──────────────────────────────'),
    '\n' + chalk.white('› Cmd       :'), chalk.green(fullCommand),
    '\n' + chalk.white('› Chat In   :'), chalk.blue(chatType),
    '\n' + chalk.white('› Status    :'), chalk.yellow(status),
    '\n' + chalk.white('› Name      :'), chalk.cyan(m.pushName),
    '\n' + chalk.white('› Sender    :'), chalk.magenta(m.sender),
        '\n' + chalk.white('──────────────────────────────') + '\n'
);
}
